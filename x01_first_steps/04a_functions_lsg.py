...
def prices_list(name, price):
    ...
    result = []
    for i in range(1, 11):
        result.append(str(i)+" * "+name+": "+str(i*price))
    return result
    #return [str(i)+" * "+name+": "+str(i*price) for i in range(1, 11)]
...
def check_euro_serial(serial):
    mod_serial = str(ord(serial[0])-64)+serial[1:]
    try:
        liste = [int(i) for i in list(mod_serial)]
    except:
        print("no serial")
        pass
    quer = sum(liste[:-1])
